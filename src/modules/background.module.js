...
export default class BackgroundModule extends Module {
    ...
    #drawCircles = (item) => {
        ...
        t = Math.sin(2+time * this.#step); // can use 0.1 0.003
        ...
        for(let theta = 0; theta < totalAngle; theta+=aStep){
            ...
            if(theta > 0){
                let hue = ((theta / Math.PI) * 180)*this.#color;
                this.ctx.beginPath();
                this.ctx.moveTo(x, y+20);
                this.ctx.lineTo(px, py+20);
                let gradient = this.ctx.createLinearGradient(200,0, 220,0);
                gradient.addColorStop(0, this.#background[0]);
                gradient.addColorStop(1, this.#background[1]);
